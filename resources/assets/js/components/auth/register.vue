...
<div class="md:bg-white container md:mx-4 md:mx-auto md:shadow-lg rounded md:mt-32 flex md:flex-row flex-col md:max-w-2xl lg:max-w-4xl justify-between">
  ...
  <div class="w-full md:bg-indigo-100 md:w-1/2 p-8">
    <form role="form" method="POST" :action="url">
      ...
      <div class="">
        <p class="py-4">
          <input id="name" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-800" type="text" name="name" placeholder="Nombre Completo">
          <span class="text-red-700 block pt-2" v-if="errors">{{ errors.name }}</span>
        </p>
        <p class="py-4">
          <input id="username" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-800" type="text" name="username" placeholder="Usuario">
        ...
        <p class="py-4">
          <button type="submit" class="bg-indigo-500 hover:bg-indigo-600 text-white text-xl font-medium py-2 px-4 rounded">Registrarse</button>
        </p>
      </div>
    </form>
  </div>
