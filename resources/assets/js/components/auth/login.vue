...
<div class="md:bg-white container md:mx-4 md:mx-auto md:shadow-lg rounded md:mt-32 flex md:flex-row flex-col md:max-w-2xl lg:max-w-4xl justify-between">
  ...
  <div class="w-full md:bg-indigo-100 md:w-1/2 p-8">
    <form method="POST" :action="url">
      ...
      <p class="py-4">
        <input id="email" aria-label="email" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-800" type="text" name="email" placeholder="Correo Electronico" :value="oldEmail" autofocus>
        <span class="text-red-400 block pt-2" v-if="errors.email">{{ errors.email[0] }}</span>
      </p>
      <p class="py-4">
        <input id="password" aria-label="password" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-800" type="password" name="password" placeholder="Contraseña">
      </p>
      ...
      <p class="py-4">
        <button type="submit" class="bg-indigo-500 hover:bg-indigo-600 text-white text-xl font-medium py-2 px-4 rounded">ACEDER</button>
      </p>
      <a href="password/reset" class="no-underline text-indigo-500 text-sm">¿Olvido su contraseña?</a>
    </form>
  </div>
</div>
</template>
